...
if($errore == "")
{
	$esperienze_lavorative = htmlspecialchars(strip_tags($parametri->esperienze_lavorative));
    $nome = $parametri->nome;
	...
	$titolo_studio = $parametri->titolo_studio;
	...
	//QUERY INSERT
	$sql = "INSERT INTO utente (nome, cognome, data_nascita, email, tel, sesso, titolo_studio, esperienze_lavorative, conoscenze_linguistiche, competenze, password, cap, citta, provincia, indirizzo, regione, nazione, saldo_tempo) VALUES ('$nome', '$cognome', '$data_nascita', '$email', '$tel', '$sesso', '$titolo_studio', '$esperienze_lavorative', '$conoscenze_linguistiche', '$competenze', '$password', '$cap', '$citta', '$provincia', '$indirizzo', '$regione', '$nazione', '$saldo_tempo')";
	$result = mysqli_query($conn, $sql);

	//Invio dati Object alert
	if ($result) echo json_encode(array("status"=>"ok","header"=>"AVVISO","message" => "Dati inseriti correttamente"));
            else echo json_encode(array("status"=>"no","header"=>"AVVISO ERRROR","message" => "ERRORE: Dati NON inseriti! <br>".$sql));	 
}
